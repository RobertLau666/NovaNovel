...
def save_outline_to_excel(outline: Dict, novel_dir: str, novel_title: str) -> str:
    ...
    if os.path.exists(excel_path):
        try:
            ...
            for idx, row in old_df.iterrows():
                # ✅ 使用统一的判断函数
                if is_chapter_done(row.get('chapter_done', 0)):
...
# ======================== 进度读取 ========================
def is_chapter_done(val) -> bool:
    """健壮的 chapter_done 判断，兼容 int/float/string"""
    if pd.isna(val):
        return False
    try:
...
def load_progress(excel_path: str) -> Dict:
    ...
    try:
        ...
        for idx, row in df.iterrows():
            if is_chapter_done(row.get('chapter_done')):
...
def process_single_task(task: Dict, task_id: int, csv_path: str) -> bool:
    ...
    if not outline:
        ...
        outline = generate_global_settings(task)
        if not outline: 
            logger.error("宏观设定生成失败")
            return False
        # ✅ 立即保存宏观设定，防止后续崩溃丢失
        with open(outline_path, 'w', encoding='utf-8') as f:
            json.dump(outline, f, ensure_ascii=False, indent=2)
        logger.info(f"💾 宏观设定已保存: {outline_path}")
    ...
    # ✅ 大纲完成后，更新 CSV 中的 outline_done 标记
    update_task_csv(csv_path, task_id, outline_done=1)
...
# ======================== 辅助函数 ========================
def update_task_csv(csv_path: str, task_id: int, status: int = None, 
                    outline_done: int = None, gen_start: bool = False, gen_end: bool = False):
    """
    更新任务CSV状态
    - status: 任务状态 (0=待处理, 1=进行中, 2=完成, 3=失败)
    - outline_done: 大纲完成标记 (0/1)
    - gen_start: 是否记录开始时间
    - gen_end: 是否记录结束时间
    """
    try:
...
def main():
    ...
    for idx, row in df.iterrows():
        ...
        if row.get('status') == 2: 
            logger.info(f"跳过已完成任务: task_id={tid}")
            continue
        
        # 记录开始时间和状态
        update_task_csv(args.file, tid, status=1, gen_start=True)
        success = process_single_task(row.to_dict(), tid, args.file)
        # 记录结束时间和状态
        update_task_csv(args.file, tid, status=2 if success else 3, gen_end=True)
